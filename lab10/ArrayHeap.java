...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        if (inBounds(ri)) {
            if ((contents[ri] == null) && (contents[li] == null)) {
                return;
            }
            int min = min(li, ri);
            Node c = contents[min];
    ...
    private void priorityHelper(int index, Node r) {
        Node n = contents[index];
        if (r.item().equals(n.item())) {
            contents[index] = r;
            if (n.priority() > r.priority()) {
                swim(index);
            } else if (n.priority() < r.priority()) {
                sink(index);
            }
        } else {
            int li = leftIndex(index);
            int ri = rightIndex(index);
            if ((inBounds(li)) && contents[li] != null) {
                priorityHelper(li, r);
            }
            if ((inBounds(ri)) && contents[ri] != null) {
                priorityHelper(ri, r);
            }
        }
    }
    ...
    @Test
    public void testInsertRemoveOnce() {
        ArrayHeap<String> pq = new ArrayHeap<>();
        pq.insert("c", 3);
        String removed = pq.removeMin();
        assertEquals("a", removed);
